...
export default function Profile(): JSX.Element {
  ...
  const [username, setUsername] = useState<string>(""); // still used for order display if available
        ...
        const normalized: Order[] = (arr || [])
          .map((o: any) => {
            ...
            const items: Item[] = (itemsRaw || []).map((it: any) => ({
              ...
              name: it.name ?? it.title ?? it.product_name ?? it.productTitle ?? `Product ${it.product_id ?? "?"}`,
              price: Number(it.price ?? it.unit_price ?? 0) || 0,
              quantity: Number(it.quantity ?? it.qty ?? 1) || 1,
              line_total: Number(it.line_total ?? it.total ?? (Number(it.price ?? 0) * Number(it.quantity ?? 1))) || 0,
              image: it.image ?? it.product_image ?? (it.product && it.product.image) ?? null,
            }));
  ...
  useEffect(() => {
    const openOrders = (location.state as any)?.openOrders;
    if (openOrders && ordersRef.current) {
  ...
  const downloadOrderCSV = (o: Order) => {
    const rows: string[][] = [
      ...
      [String(o.order_id), String(o.created_at ?? ""), String(o.username ?? username), String(o.status ?? ""), String(o.total ?? "")],
      [],
    ...
    downloadCSV(`order-${o.order_id}.csv`, rows);
  };
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800">
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left column: only actions (no username or user id shown) */}
          <Card className="p-6 lg:col-span-1">
            <div className="flex flex-col gap-4">
              <Button onClick={() => navigate("/profile", { state: { openOrders: true } })} className="w-full">
                My Orders
              </Button>
                  ...
                  <div key={String(o.order_id)} className="flex items-start justify-between border rounded p-4 bg-white">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-700 mt-2">
                        <strong>User:</strong> {o.username ?? username} • <strong>Items:</strong> {o.items.length}
                      </div>
                    </div>
          ...
          <div className="relative max-w-2xl w-full bg-white rounded-xl shadow-2xl p-6 z-10">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-sm text-gray-700 mt-2">Customer: {selected.username ?? username}</div>
              </div>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <Button onClick={() => downloadOrderCSV(selected)}>Download CSV</Button>
            </div>
          </div>
        </div>
      )}
